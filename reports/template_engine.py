...
class ReportGenerator:
    ...
    def generate_markdown(
        ...
        lines = [
            f"# Analysis Report: {paper.get('title', 'Scientific Paper')}",
            f"",
        ...
        if repo:
            ...
            lines.append(f"- **Primary Language:** {repo.get('primary_language', 'Unknown')}")
            lines.append(f"- **Total Files:** {repo.get('total_files', 0)}")
            lines.append(f"- **Setup Complexity:** {repo.get('setup_complexity', 'Medium')}")
            lines.append("")

        lines.append("---")
        lines.append("")
        lines.append("## Concept-to-Code Mappings")
        lines.append("")

        if mappings:
            lines.append("| Concept | Code Element | File | Confidence |")
            lines.append("|---------|--------------|------|------------|")
            for m in mappings:
                concept = m.get("concept_name", m.get("concept", ""))
                code_elem = m.get("code_element", "")
                file_path = m.get("code_file", m.get("file_path", ""))
                confidence = m.get("confidence", 0)
                lines.append(f"| {concept} | `{code_elem}` | {file_path} | {int(confidence * 100)}% |")
            lines.append("")
        else:
        ...
        if code_results:
            ...
            lines.append(f"**Summary:** {success_count}/{len(code_results)} tests passed")
            lines.append("")
            for result in code_results:
                status = "✅ PASS" if result.get("success") else "❌ FAIL"
                name = result.get("concept", result.get("script_name", "Test"))
                lines.append(f"### {name} - {status}")
                lines.append("")
                if result.get("stdout"):
                    lines.append("```")
                    lines.append(result["stdout"][:500])
                    lines.append("```")
                if result.get("error") and not result.get("success"):
                    lines.append(f"**Error:** {result['error']}")
                lines.append("")
        else:
            lines.append("*No code execution results.*")
            lines.append("")

        lines.append("---")
        lines.append("")
        lines.append("*Generated by Scientific Paper Analysis System*")
